#!/usr/bin/env bash
# Drives the full climate experiment pipeline once per confidence threshold:
...
uv run python experiments/climate/mine_rules.py \
...
for CONF in "${CONFIDENCE_THRESHOLDS[@]}"; do
    ...
    uv run python experiments/climate/alphas.py --confidence "$CONF" --cpu-count 12 &
    ALPHAS_PID=$!
    uv run python experiments/climate/ids_lambda_search.py --confidence "$CONF" &
    IDS_SEARCH_PID=$!
    wait "$ALPHAS_PID"
    wait "$IDS_SEARCH_PID"

    # Stage 2: depends on alphas.py's output.
    uv run python experiments/climate/select_alphas.py --confidence "$CONF"
    ...
    uv run python experiments/climate/max_rules.py --confidence "$CONF" --cpu-count 2 &
    MAX_RULES_PID=$!
    uv run python experiments/climate/lambda.py --confidence "$CONF" --cpu-count 12 &
    LAMBDA_PID=$!
    wait "$MAX_RULES_PID"
    wait "$LAMBDA_PID"
