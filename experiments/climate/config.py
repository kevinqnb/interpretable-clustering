####################################################################################################
# Shared, unchanging parameters for the climate experiment pipeline.
#
...
# count; override via CLIMATE_TOTAL_CPU_COUNT (or experiment_runner.py's
# --total-cpu-count) to reserve some cores for other work.
TOTAL_CPU_COUNT = int(os.environ.get('CLIMATE_TOTAL_CPU_COUNT', os.cpu_count() or 1))
...
# stage script standalone uses the whole machine. experiment_runner.py overrides
# this per subprocess via CLIMATE_CPU_COUNT, dividing TOTAL_CPU_COUNT across
# however many scripts it launches concurrently at each stage (e.g. max_rules.py,
# lambda.py, and confidence.py all run at once in the final stage, so each gets
# roughly a third of the budget rather than all of it) so no stage oversubscribes.
CPU_COUNT = int(os.environ.get('CLIMATE_CPU_COUNT', TOTAL_CPU_COUNT))
...
RULES_DIR = 'data/experiments/climate/rules/'
ALPHAS_DIR = 'data/experiments/climate/alphas/'
