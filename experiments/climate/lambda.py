...
args = parse_experiment_args(confidence_default=0.75, cpu_count_default=24)
...
with open(f"data/experiments/climate/alphas/selected_alphas_resub_conf_{tag}.json") as f:
...
outfile = 'data/experiments/climate/lambda/'
...
ensemble_rules = load_rules(f'data/experiments/climate/rules/ensemble_rules_conf_{tag}.pkl')

with open(f'data/experiments/climate/rules/ensemble_labels_conf_{tag}.pkl', 'rb') as f:
...
# IDS:
with open(f'data/experiments/climate/rules/ids_lambdas_conf_{tag}.json') as f:
    ids_lambdas = json.load(f)
if isinstance(ids_lambdas, dict):
    ids_lambdas = list(ids_lambdas.values())

_ids_cache_path = f'data/experiments/climate/rules/ids_coverage_cache_ensemble_conf_{tag}.pkl'
