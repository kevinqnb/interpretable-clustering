#!/usr/bin/env python3
####################################################################################################
# End-to-end runner for the mnist experiment pipeline, at the single default confidence
...
#         (mnist splits max_rules.py/lambda.py across companion scripts --
...
from experiments.mnist.config import TOTAL_CPU_COUNT

MNIST_DIR = PROJECT_ROOT / "experiments" / "mnist"
...
def _spawn(script_name: str, env: dict) -> subprocess.Popen:
    ...
    # paths (e.g. 'data/experiments/mnist/rules/') and never chdir's itself.
    return subprocess.Popen(
        [sys.executable, str(MNIST_DIR / script_name)],
...
def _run_stage(scripts: list[str], total_cpu_count: int) -> None:
    ...
    (via the MNIST_CPU_COUNT env var, which config.py's CPU_COUNT reads).
    """
    per_script = max(1, total_cpu_count // len(scripts))
    env = dict(os.environ) | {"MNIST_CPU_COUNT": str(per_script)}
...
def _run_family(scripts: list[str], cpu_count: int) -> None:
    ...
    MNIST_CPU_COUNT, so passing it along is harmless overhead, not lost capacity.
    """
    env = dict(os.environ) | {"MNIST_CPU_COUNT": str(cpu_count)}
...
def main():
    parser = argparse.ArgumentParser(
        description="Run the mnist experiment pipeline end-to-end."
    )
    parser.add_argument(
        ...
        help=(
            ...
            "experiments/mnist/config.py's TOTAL_CPU_COUNT, which itself "
    ...
    _run_stage(["max_rules_ids_alt.py", "max_rules_combine_ids_alt.py"], total_cpu_count)
