...
# Identical to lambda_exkmc.py, except alpha is fixed to 0 for every module instead of being
# loaded from select_alphas.py's elbow-selected 'selected_alphas*.json' -- so this doesn't depend
# on select_alphas.py having been run at all. `defaultdict(float)` stands in for the per-module
# dict lambda_exkmc.py loads from disk: every module_name lookup below
# (`fixed_parameters['alpha'][...]`) resolves to 0.0 regardless of key. This matters here even
# though ExKMC itself has no alpha_val parameter: alpha still feeds the lambda*/lambda-grid probe
# below, which must land on the exact same grid as lambda_alpha_zero.py's so ExKMC's single fit
# can be broadcast under the same `all_lambda_values` keys once merged by
# lambda_combine_alpha_zero.py.
fixed_parameters['alpha'] = defaultdict(float)

decision_info_dict_directory = RULES_DIR

outfile = LAMBDA_DIR
# This output ref differs from lambda_exkmc.py's so this script's (alpha=0) results don't clobber
# lambda_exkmc.py's own (elbow-selected alpha) output.
