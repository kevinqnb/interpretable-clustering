#!/usr/bin/env bash
# Drives the full mnist experiment pipeline once per confidence threshold:
...
#
# mnist splits max_rules.py/lambda.py across companion _exkmc/_exp scripts (some
...
uv run python experiments/mnist/mine_rules.py \
...
for CONF in "${CONFIDENCE_THRESHOLDS[@]}"; do
    ...
    uv run python experiments/mnist/alphas.py --confidence "$CONF" --cpu-count 12 &
    ALPHAS_PID=$!
    uv run python experiments/mnist/ids_lambda_search.py --confidence "$CONF" &
    IDS_SEARCH_PID=$!
    wait "$ALPHAS_PID"
    wait "$IDS_SEARCH_PID"

    # Stage 2: depends on alphas.py's output.
    uv run python experiments/mnist/select_alphas.py --confidence "$CONF"
    ...
    (
        uv run python experiments/mnist/max_rules.py --confidence "$CONF" --cpu-count 6
        uv run python experiments/mnist/max_rules_exkmc.py --confidence "$CONF" --cpu-count 1
        uv run python experiments/mnist/max_rules_exp.py --confidence "$CONF" --cpu-count 1
        uv run python experiments/mnist/max_rules_combine.py --confidence "$CONF"
    ) &
    MAX_RULES_PID=$!

    (
        uv run python experiments/mnist/lambda.py --confidence "$CONF" --cpu-count 6
        uv run python experiments/mnist/lambda_exkmc.py --confidence "$CONF" --cpu-count 1
        uv run python experiments/mnist/lambda_exp.py --confidence "$CONF" --cpu-count 1
        uv run python experiments/mnist/lambda_combine.py --confidence "$CONF"
    ) &
    LAMBDA_PID=$!
