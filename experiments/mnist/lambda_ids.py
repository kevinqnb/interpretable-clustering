...
####################################################################################################
# This is the IDS-only counterpart to lambda.py -- part of the same per-model split as
...
#
# IDS's fitted solution doesn't depend on PEC's lambda at all (it never varies with it), so it's
# refit once per trial seed here and the trial-aggregated result is broadcast across every value
# in `all_lambda_values` -- the union of every objective's lambda grid.
#
...
# concurrently with max_rules.py/max_rules_ids.py or lambda.py by hand (rather than through
# experiment_runner.py, which sequences ids_lambda_search.py before either), make sure that cache
# already exists first -- otherwise multiple processes may race to build and write the same
# ~GB-scale file.
...
if os.path.exists(_ids_cache_path):
    ...
    # Built exactly the way ids_lambda_search.py builds it -- see max_rules.py's identical block
...
####################################################################################################
# Objectives for Decision Set Clustering (needed only to reproduce the lambda grid IDS's result
...
# Union of every objective's lambda grid -- used to broadcast IDS's trial-aggregated result
# (which doesn't depend on lambda or the objective at all) across every lambda value any
...
# No joblib-dispatched modules here -- IDS is refit once per trial seed single-process below
# (see lambda.py's "Stochastic module trials" for why: joblib worker processes do not inherit
# this script's seeded global NumPy state, which would make results irreproducible for this
# randomized module). `module_list` is left empty so `exp.run()` still produces the baseline
...
#
# IDS has a fitted solution that depends on randomness. Rather than record one arbitrarily-seeded
# fit, it is refit once per seed in `trial_seeds` and the results across trials are aggregated
# into {'mean', 'std', 'values'} via `aggregate_trials` (see experiments/modules.py). This runs
# single-process (not through `Experiment`'s joblib dispatch) specifically so each trial's
# explicit seed is what controls its randomness. Since IDS doesn't vary with lambda, it's fit
# once per trial seed (not once per lambda value) and the trial-aggregated result is broadcast
# across `all_lambda_values`.
...
print(f"Fitting IDS across {n_trials} trials...")
