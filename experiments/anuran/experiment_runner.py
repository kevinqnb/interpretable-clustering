#!/usr/bin/env python3
####################################################################################################
# End-to-end runner for the anuran experiment pipeline, at the single default confidence
...
def _run_stage(scripts: list[str], total_cpu_count: int) -> None:
    ...
    (via the ANURAN_CPU_COUNT env var, which config.py's CPU_COUNT reads) -- so a
...
def main():
    parser = argparse.ArgumentParser(
        description="Run the anuran experiment pipeline end-to-end."
    )
    parser.add_argument(
        ...
        help=(
            ...
            "experiments/anuran/config.py's TOTAL_CPU_COUNT, which itself "
