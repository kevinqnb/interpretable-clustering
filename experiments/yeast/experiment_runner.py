...
from experiments.yeast.config import TOTAL_CPU_COUNT

YEAST_DIR = PROJECT_ROOT / "experiments" / "yeast"
...
def _spawn(script_name: str, env: dict) -> subprocess.Popen:
    ...
    # paths (e.g. 'data/experiments/yeast/rules/') and never chdir's itself.
    return subprocess.Popen(
        [sys.executable, str(YEAST_DIR / script_name)],
...
def _run_stage(scripts: list[str], total_cpu_count: int) -> None:
    ...
    (via the YEAST_CPU_COUNT env var, which config.py's CPU_COUNT reads) -- so a
    ...
    env = dict(os.environ) | {"YEAST_CPU_COUNT": str(per_script)}
