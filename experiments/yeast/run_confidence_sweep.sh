#!/usr/bin/env bash
# Drives the full yeast experiment pipeline once per confidence threshold:
...
uv run python experiments/yeast/mine_rules.py \
...
for CONF in "${CONFIDENCE_THRESHOLDS[@]}"; do
    ...
    uv run python experiments/yeast/alphas.py --confidence "$CONF" --cpu-count 12 &
    ALPHAS_PID=$!
    uv run python experiments/yeast/ids_lambda_search.py --confidence "$CONF" &
    IDS_SEARCH_PID=$!
    wait "$ALPHAS_PID"
    wait "$IDS_SEARCH_PID"

    # Stage 2: depends on alphas.py's output.
    uv run python experiments/yeast/select_alphas.py --confidence "$CONF"
    ...
    uv run python experiments/yeast/max_rules.py --confidence "$CONF" --cpu-count 6 &
    MAX_RULES_PID=$!
    uv run python experiments/yeast/lambda.py --confidence "$CONF" --cpu-count 6 &
    LAMBDA_PID=$!
    wait "$MAX_RULES_PID"
    wait "$LAMBDA_PID"
