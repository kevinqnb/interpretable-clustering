...
from experiments.yeast.config import (
...
####################################################################################################
# This is a PEC-lazy-greedy-only counterpart to max_rules.py. Unlike mnist/fashion, this dataset
# doesn't have a full per-model split -- max_rules.py itself is still the single job that produces
# the saved solution (it now fits PEC's lazy-greedy modules directly too, see its
# dscluster_module_list loop). This script exists purely so lazy-greedy results can be added to an
# *already-completed* max_rules.py run without rerunning the whole thing (CBA/IDS/Decision-Tree/
# CN2/PEC-distorted-greedy all included) just to pick up the new lazy-greedy modules. See
# max_rules_combine.py, which merges this script's output into the existing saved solution.
#
# NOTE: this uses its own outfile_ref ('_pec_lazy', not OUTFILE_REF) so it never collides with
# max_rules.py's own output file if both exist on disk at once.
...
# as max_rules.py does -- purely so the lazy-greedy modules below can be fit at the *same* lambda
# the distorted-greedy modules use. This makes lazy-greedy vs distorted-greedy a genuine
# apples-to-apples comparison of the two selection algorithms rather than two differently-tuned
# models. See max_rules.py's identical probe block for the full rationale on why this is cheap
# (reuses the precomputed coverage/cost caches).
...
for obj_name, obj_params in objective_dict.items():
    for rule_miner_name, (rule_miner, rules, rule_labels) in rule_miner_dict.items():
        ...
        # Only the lazy-greedy module is built here -- the distorted-greedy counterpart already
        # lives in the existing saved solution from max_rules.py. Named with an '; lazy-greedy'
        # suffix so it never collides with max_rules.py's distorted-greedy module of the same
        # objective.
        lazy_dsclust_params = {
