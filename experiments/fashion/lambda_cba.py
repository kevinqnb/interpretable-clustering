...
####################################################################################################
# This is the CBA-only counterpart to lambda.py -- part of the same per-model split as
...
#
# CBA's single fit doesn't depend on lambda at all (see lambda.py's note on this), so it's fit
...
# reproduce that grid identically, even though CBA's own fit doesn't touch the objectives.
...
####################################################################################################
# Objectives for Decision Set Clustering (needed only to reproduce the lambda grid CBA's result
...
# Union of every objective's lambda grid -- used to broadcast CBA's single fit (which doesn't
...
module_list = [(cba_mod, {all_lambda_values: cba_shared_params})]
