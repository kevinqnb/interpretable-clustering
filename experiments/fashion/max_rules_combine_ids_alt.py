...
# Alternate-IDS counterpart to max_rules_combine.py: identical merge logic, except the 'ids' part
# points at max_rules_ids_alt.py's output (IDS tuned by coordinate ascent to maximize the PEC
# objective, rather than max_rules_ids.py's held-out-AUC-tuned IDS) and the combined output is
# written to its own '_ids_alt'-suffixed ref so it never overwrites the canonical combined file.
# Every other part is reused as-is from whatever max_rules_combine.py already merged in --
# ExKMC/CBA/CN2/Decision-Tree/PEC don't depend on which IDS lambda search ran, so they aren't
# recomputed here.
#
...
#   - max_rules_{cba,cn2,dtree,pec,pec_lazy}.py: the per-model split (see those scripts).
...
#   - max_rules_ids_alt.py: IDS tuned to maximize the PEC objective (see ids_lambda_search_alt.py).
...
#     Note this fallback's IDS is the AUC-tuned one, not the alt one -- see the stale-modules
#     warning below.
