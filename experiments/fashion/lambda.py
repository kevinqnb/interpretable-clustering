...
# Note: unlike max_rules.py (which splits Exp-Tree/ExKMC into separate
# max_rules_exp.py/max_rules_exkmc.py scripts to avoid refitting expensive models
# once per rule budget r -- see experiments/README.md), this experiment only ever
# fits ExKMC/Exp-Tree once (per trial, for Exp-Tree) regardless of the lambda
# sweep, so everything is cheap enough to keep in a single file.
seed = 342
...
outfile = 'data/experiments/fashion/lambda/'
outfile_ref = '_resub'
...
with open('data/experiments/fashion/rules/ensemble_labels.pkl', 'rb') as f:
...
# internal randomness given fixed inputs.
#
...
# budget, so its single fit's result is recorded under every lambda label. Unlike
# max_rules_exp.py, which split this into its own file because it was refit once
# per rule budget r across 10 trials each, this experiment only needs 10 total
# fits regardless of the lambda sweep, so it's cheap enough to keep here.)
...
# ExKMC
# (Unlike max_rules_exkmc.py, which refit this once per rule budget r and split
# it into its own file to avoid 7 refits of an expensive model, this experiment
# only needs a single fit, so it's cheap enough to keep in the main module list.)
exkmc_shared_params = {
