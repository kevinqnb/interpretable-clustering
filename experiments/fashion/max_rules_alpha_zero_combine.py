...
# alpha=0 counterpart of max_rules_combine.py, restricted to the two alpha-zero parts that
# actually exist (see max_rules_alpha_zero.py / max_rules_exkmc_alpha_zero.py) -- there is no
# alpha-zero equivalent of the cba/cn2/dtree/ids/pec_lazy splits or the monolithic max_rules.py
# fallback, since those were never asked for.
#   - max_rules_alpha_zero.py: alpha=0 counterpart of the dscluster split (PEC/CBA/Decision-Tree/
#     IDS/CN2, everything max_rules.py itself fits) -- the primary part, required.
#   - max_rules_exkmc_alpha_zero.py: alpha=0 counterpart of the ExKMC split -- optional add-on,
#     since alpha never affects ExKMC (it has no alpha_val parameter; see that script's comment)
#     and merging it in just fills out the comparison table.
DSCLUSTER_PART = 'dscluster_alpha_zero (max_rules_alpha_zero.py)'
part_refs = {
...
if DSCLUSTER_PART in missing_parts:
    raise FileNotFoundError(
        f"{os.path.join(max_rules_dir, 'exp_dscluster' + OUTFILE_REF + '_alpha_zero.json')} "
        f"(max_rules_alpha_zero.py's output) was not found. This script only ADDS "
        f"max_rules_exkmc_alpha_zero.py's ExKMC results on top of that saved solution -- it "
        f"doesn't recompute anything else. Run max_rules_alpha_zero.py first if that file "
        f"doesn't exist yet."
    )
...
if 'exkmc_alpha_zero' in missing_parts:
    print(
        ...
        f"{os.path.join(max_rules_dir, 'exp_exkmc' + OUTFILE_REF + '_alpha_zero.json')} "
        f"(max_rules_exkmc_alpha_zero.py's output) was not found. Run "
        f"max_rules_exkmc_alpha_zero.py first if you want ExKMC included. Saving the "
