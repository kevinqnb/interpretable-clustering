####################################################################################################
# Shared, unchanging parameters for the fashion experiment pipeline.
#
...
# count; override via FASHION_TOTAL_CPU_COUNT (or experiment_runner.py's
# --total-cpu-count) to reserve some cores for other work.
TOTAL_CPU_COUNT = int(os.environ.get('FASHION_TOTAL_CPU_COUNT', os.cpu_count() or 1))
...
# FASHION_CPU_COUNT, dividing TOTAL_CPU_COUNT across however many scripts/families
# it launches concurrently at each stage so no stage oversubscribes.
CPU_COUNT = int(os.environ.get('FASHION_CPU_COUNT', TOTAL_CPU_COUNT))
...
ALPHAS_DIR = 'data/experiments/fashion/alphas/'
