...
# Note: ExKMC, being deterministic given a fixed kmeans, is fit in the separate
# `max_rules_exkmc.py` script -- see experiments/README.md's note on why
# mnist/fashion split max_rules across files.
seed = SEED
...
# Identical to max_rules.py, except alpha is fixed to 0 for every module instead of being loaded
...
# max_rules.py loads from disk: every module_name lookup below (`fixed_parameters['alpha'][...]`)
...
# This output ref differs from max_rules.py's so this script's (alpha=0) results don't clobber
# max_rules.py's own (elbow-selected alpha) output.
...
# any seed -- the per-trial seed is injected at fit time. PEC, CBA, and CN2
# have no internal randomness given fixed inputs, so they keep the original
# single-fit-per-parameter-value treatment via `Experiment`. ExKMC is handled in
# the separate `max_rules_exkmc.py` script (see `max_rules_combine.py`).
#
# NOTE: Shallow-Tree used to be fit here too, and Exp-Tree used to be fit in its
# own `max_rules_exp.py` script (merged in by `max_rules_combine.py`), but
# neither appears in examples/experiments.ipynb's `comparison_modules`. Both
# dropped -- max_rules_exp.py and lambda_exp.py (its lambda.py counterpart) are
# deleted entirely, and max_rules_combine.py/lambda_combine.py no longer merge
# an "_exp" ref.
...
# randomized modules). ExKMC is fit in the separate `max_rules_exkmc.py` script.
# CN2 is also handled separately below, via `fit_cn2_varying` -- its
# induction doesn't depend on the rule budget r at all, so sweeping it
# through Experiment's per-(module, param) joblib dispatch the way CBA
# is swept would rerun the same expensive induction once per r for an
...
# joblib dispatch, the way CBA is swept, would rerun the same induction
# from scratch once per rule budget in n_rules_list for an otherwise-identical
# result. CN2 is deterministic (no seed dependence), so it's fit outside
# `Experiment.run()` here: induce once, then finalize + measure cheaply per
# budget.
