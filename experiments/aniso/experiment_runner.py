#!/usr/bin/env python3
####################################################################################################
# End-to-end runner for the aniso experiment pipeline, at the single default confidence
...
#      -> [ max_rules.py || lambda.py || confidence.py ]       (parallel; all three depend only
...
from experiments.aniso.config import TOTAL_CPU_COUNT

ANISO_DIR = PROJECT_ROOT / "experiments" / "aniso"
...
def _spawn(script_name: str, env: dict) -> subprocess.Popen:
    ...
    # paths (e.g. 'data/experiments/aniso/rules/') and never chdir's itself.
    return subprocess.Popen(
        [sys.executable, str(ANISO_DIR / script_name)],
...
def _run_stage(scripts: list[str], total_cpu_count: int) -> None:
    ...
    (via the ANISO_CPU_COUNT env var, which config.py's CPU_COUNT reads) -- so a
    ...
    env = dict(os.environ) | {"ANISO_CPU_COUNT": str(per_script)}
...
def main():
    parser = argparse.ArgumentParser(
        description="Run the aniso experiment pipeline end-to-end."
    )
    parser.add_argument(
        ...
        help=(
            ...
            "experiments/aniso/config.py's TOTAL_CPU_COUNT, which itself "
