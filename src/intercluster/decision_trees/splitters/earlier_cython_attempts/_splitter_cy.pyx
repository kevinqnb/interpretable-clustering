...
cdef float cost(
    ...
    if len(indices) == 0:
        ...
        unique_labels, counts = np.unique(sublabels, return_counts=True)
        entropy_score = entropy(counts/len(sublabels), base=2)
...
cdef float gain(
    ...
    left_cost = len(left_indices)/total_indices * cost(left_indices, y)
    right_cost = len(right_indices)/total_indices * cost(right_indices, y)
...
def split_cy(
    ...
    cdef float parent_cost = cost(indices, y)
    ...
    for i in range(d):
        ...
        for j in range(unique_values.shape[0]):
            ...
            if (len(left_indices) < min_points_leaf or 
                ...
                gain_val = gain(X.shape[0], y, left_indices, right_indices, parent_cost)
    ...
    condition = LinearCondition(
        features = np.array([best_condition[0]]),
        weights = np.array([1]),
        threshold = best_condition[1],
        direction = -1
    )
    return best_gain_val, condition
