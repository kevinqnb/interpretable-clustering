...
def test_unsupervised_cost(unsupervised_dataset):
    S1 = UnsupervisedSplitter(norm = 1)
    S2 = UnsupervisedSplitter(norm = 2)
    S1.fit(X = unsupervised_dataset)
    S2.fit(X = unsupervised_dataset)
    
    assert np.isclose(S1.cost(np.arange(len(unsupervised_dataset))), 400, atol = 1e-5)
    assert np.isclose(S2.cost(np.arange(len(unsupervised_dataset))), 800, atol = 1e-5)
    
    
def test_unsupervised_split(unsupervised_dataset):
    S = UnsupervisedSplitter(norm = 2)
    ...
    for i in range(samples):
        gain, split_info = S.split(np.arange(len(unsupervised_dataset)))
